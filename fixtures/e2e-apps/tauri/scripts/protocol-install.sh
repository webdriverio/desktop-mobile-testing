#!/bin/bash
# Protocol handler installer - detects platform and runs appropriate setup
...
case "$OS_TYPE" in
  linux*)
    echo "[protocol-install] Detected Linux - running Tauri protocol handler setup..."
  ...
  mingw*|msys*|cygwin*|windows*)
    ...
    if command -v pwsh &> /dev/null; then
      ...
      pwsh -ExecutionPolicy Bypass -File ./fixtures/e2e-apps/tauri/scripts/setup-protocol-handler.ps1
    # Check for Windows PowerShell (powershell)
      ...
      echo "  pwsh -ExecutionPolicy Bypass -File ./fixtures/e2e-apps/tauri/scripts/setup-protocol-handler.ps1"
      exit 1
    fi
    ;;
  *)
    ...
    echo "  Linux:   ./fixtures/e2e-apps/tauri/scripts/setup-protocol-handler.sh"
    echo "  Windows: ./fixtures/e2e-apps/tauri/scripts/setup-protocol-handler.ps1"
