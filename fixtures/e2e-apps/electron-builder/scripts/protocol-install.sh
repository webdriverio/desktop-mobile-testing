#!/bin/bash
# Protocol handler installer for Electron Builder - detects platform
...
case "$OS_TYPE" in
  linux*)
    echo "[protocol-install] Detected Linux - running Electron Builder protocol handler setup..."
    chmod +x ./fixtures/e2e-apps/electron-builder/scripts/setup-protocol-handler.sh
    ./fixtures/e2e-apps/electron-builder/scripts/setup-protocol-handler.sh
    ;;
  darwin*)
  ...
  mingw*|msys*|cygwin*|windows*)
    ...
    if command -v pwsh &> /dev/null; then
      ...
      pwsh -ExecutionPolicy Bypass -File ./fixtures/e2e-apps/electron-builder/scripts/setup-protocol-handler.ps1
    # Check for Windows PowerShell (powershell)
    elif command -v powershell &> /dev/null; then
      echo "[protocol-install] Windows PowerShell found - running setup..."
      powershell -ExecutionPolicy Bypass -File ./fixtures/e2e-apps/electron-builder/scripts/setup-protocol-handler.ps1
    else
      ...
      echo "  pwsh -ExecutionPolicy Bypass -File ./fixtures/e2e-apps/electron-builder/scripts/setup-protocol-handler.ps1"
      exit 1
    fi
    ;;
  *)
    ...
    echo "  Linux:   ./fixtures/e2e-apps/electron-builder/scripts/setup-protocol-handler.sh"
    echo "  Windows: ./fixtures/e2e-apps/electron-builder/scripts/setup-protocol-handler.ps1"
