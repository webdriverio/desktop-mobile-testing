...
The service can capture console output from both Electron's main process and renderer processes, providing visibility into your application's behavior during tests.
...
- If the CDP bridge is unavailable:
  ...
  - See [CDP bridge initialization](#cdp-bridge-cannot-be-initialized-enablenodecliinspectarguments-fuse-is-disabled) for troubleshooting
...
- ❌ Main process log capture will not work (see [Log Capture Requirements](#log-capture-requirements))
