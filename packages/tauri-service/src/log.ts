// Use the same logger approach as the Electron service
...
export function createLogger(area?: LogArea): Logger {
  ...
  const areaDebug = debug(`wdio-tauri-service${areaSuffix}`);
  const areaLogger = createWdioLogger(`tauri-service${areaSuffix}`);
